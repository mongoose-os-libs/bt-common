...
struct esp32xx_bt_gatts_service_entry {
  struct mgos_bt_uuid uuid;
  uint16_t svc_handle;
  struct esp32xx_bt_service_attr_info *attrs;
  uint16_t num_attrs;
  ...
  SLIST_ENTRY(esp32xx_bt_gatts_service_entry) next;
};
...
struct esp32xx_bt_gatts_pending_ind {
  ...
  bool is_ind;
  struct mg_str value;
  STAILQ_ENTRY(esp32xx_bt_gatts_pending_ind) next;
};

struct esp32xx_bt_gatts_pending_read {
  uint16_t handle;
  struct mbuf data;
  size_t read_offset;
  SLIST_ENTRY(esp32xx_bt_gatts_pending_read) next;
};

struct esp32xx_bt_gatts_connection_entry;

// A set of sessions is kept for each connection, one per service.
struct esp32xx_bt_gatts_session_entry {
  struct esp32xx_bt_gatts_connection_entry *ce;
  struct mgos_bt_gatts_conn gsc;
  struct esp32xx_bt_gatts_service_entry *se;
  SLIST_ENTRY(esp32xx_bt_gatts_session_entry) next;
};

// This notification mode change is awaiting completion of the security
// procedure.
struct esp32xx_bt_gatts_pending_nm {
  struct mgos_bt_gatts_notify_mode_arg nmarg;
  SLIST_ENTRY(esp32xx_bt_gatts_pending_nm) next;
};
...
struct esp32xx_bt_gatts_connection_entry {
  ...
  int ind_queue_len;
  bool ind_in_flight;
  SLIST_HEAD(sessions, esp32xx_bt_gatts_session_entry)
  sessions;  // 1 per service
  SLIST_HEAD(pending_nm, esp32xx_bt_gatts_pending_nm) pending_nm;
  SLIST_HEAD(pending_reads, esp32xx_bt_gatts_pending_read) pending_reads;
  STAILQ_HEAD(pending_inds, esp32xx_bt_gatts_pending_ind) pending_inds;
  SLIST_ENTRY(esp32xx_bt_gatts_connection_entry) next;
};

static SLIST_HEAD(s_svcs, esp32xx_bt_gatts_service_entry) s_svcs =
    SLIST_HEAD_INITIALIZER(s_svcs);
static SLIST_HEAD(s_conns, esp32xx_bt_gatts_connection_entry) s_conns =
    SLIST_HEAD_INITIALIZER(s_conns);

static void esp32xx_bt_gatts_send_next_ind_locked(
    struct esp32xx_bt_gatts_connection_entry *ce);
static void esp32xx_bt_gatts_create_sessions(
    struct esp32xx_bt_gatts_connection_entry *ce);

static void esp32xx_bt_gatts_pending_ind_free(
    struct esp32xx_bt_gatts_pending_ind *pi) {
  mg_strfree(&pi->value);
  memset(pi, 0, sizeof(*pi));
  free(pi);
}

static void esp32xx_bt_gatts_pending_read_free(
    struct esp32xx_bt_gatts_pending_read *pr) {
  mbuf_free(&pr->data);
  free(pr);
}
  ...
  switch (ctxt->op) {
    case BLE_GATT_REGISTER_OP_SVC: {
      LOG(LL_DEBUG, ("REGISTER_SVC %s sh %d",
                     esp32xx_bt_uuid_to_str(ctxt->svc.svc_def->uuid, buf),
                     ctxt->svc.handle));
      break;
    }
    case BLE_GATT_REGISTER_OP_CHR: {
      LOG(LL_DEBUG, ("REGISTER_CHR %s dh %d vh %d",
                     esp32xx_bt_uuid_to_str(ctxt->chr.chr_def->uuid, buf),
                     ctxt->chr.def_handle, ctxt->chr.val_handle));
      break;
    }
    case BLE_GATT_REGISTER_OP_DSC: {
      LOG(LL_DEBUG, ("REGISTER_DSC %s vh %d",
                     esp32xx_bt_uuid_to_str(ctxt->dsc.dsc_def->uuid, buf),
                     ctxt->dsc.handle));
      ...
      esp32xx_bt_uuid_to_mgos(ctxt->dsc.dsc_def->uuid, &desc_uuid);
      struct esp32xx_bt_service_attr_info *ai = ctxt->dsc.dsc_def->arg;
...
static struct esp32xx_bt_gatts_service_entry *find_service_by_attr_handle(
    uint16_t attr_handle, struct esp32xx_bt_service_attr_info **aip) {
  struct esp32xx_bt_gatts_service_entry *se;
  SLIST_FOREACH(se, &s_svcs, next) {
    struct esp32xx_bt_service_attr_info *ai = &se->attrs[0];
...
static struct esp32xx_bt_gatts_connection_entry *find_connection(
    uint16_t conn_id) {
  struct esp32xx_bt_gatts_connection_entry *ce = NULL;
...
static struct esp32xx_bt_gatts_session_entry *find_session(
    uint16_t conn_id, uint16_t handle,
    struct esp32xx_bt_service_attr_info **ai) {
  struct esp32xx_bt_gatts_connection_entry *ce = find_connection(conn_id);
...
static struct esp32xx_bt_gatts_session_entry *find_session_by_gsc(
    const struct mgos_bt_gatts_conn *gsc) {
  if (gsc == NULL) return NULL;
  struct esp32xx_bt_gatts_connection_entry *ce =
      find_connection(gsc->gc.conn_id);
  if (ce == NULL) return NULL;
  struct esp32xx_bt_gatts_session_entry *sse;
  SLIST_FOREACH(sse, &ce->sessions, next) {
...
static enum mgos_bt_gatt_status esp32xx_bt_gatts_call_handler(
    struct esp32xx_bt_gatts_session_entry *sse,
    struct esp32xx_bt_service_attr_info *ai, enum mgos_bt_gatts_ev ev,
    void *ev_arg) {
  struct esp32xx_bt_gatts_service_entry *se = sse->se;
...
void esp32xx_bt_gatts_close_session(
    struct esp32xx_bt_gatts_session_entry *sse) {
  struct esp32xx_bt_gatts_pending_nm *pnm, *pnmt;
  SLIST_FOREACH_SAFE(pnm, &sse->ce->pending_nm, next, pnmt) {
    memset(pnm, 0, sizeof(*pnm));
    free(pnm);
  }
  SLIST_REMOVE(&sse->ce->sessions, sse, esp32xx_bt_gatts_session_entry, next);
  esp32xx_bt_gatts_call_handler(sse, NULL, MGOS_BT_GATTS_EV_DISCONNECT, NULL);
...
static void esp32xx_bt_gatts_create_sessions(
  ...
  struct esp32xx_bt_gatts_service_entry *se;
  SLIST_FOREACH(se, &s_svcs, next) {
    struct esp32xx_bt_gatts_session_entry *sse = calloc(1, sizeof(*sse));
    if (sse == NULL) break;
    sse->ce = ce;
    sse->se = se;
    sse->gsc.gc = ce->gc;
    sse->gsc.svc_uuid = se->uuid;
    enum mgos_bt_gatt_status st = esp32xx_bt_gatts_call_handler(
        sse, NULL, MGOS_BT_GATTS_EV_CONNECT, NULL);
...
static void esp32xx_bt_gatts_send_next_ind_locked(
  ...
  struct esp32xx_bt_gatts_pending_ind *pi = STAILQ_FIRST(&ce->pending_inds);
...
static int esp32xx_bt_gatts_mtu_event(uint16_t conn_id,
                                      const struct ble_gatt_error *err,
                                      uint16_t mtu, void *arg) {
  struct esp32xx_bt_gatts_connection_entry *ce = arg;
  LOG(LL_DEBUG, ("MTU_FN %d st %d mtu %d", conn_id, err->status, mtu));
  // Peer may not support MTU exchange procedure, proceed anyway.
  if (err->status == 0) {
    ce->gc.mtu = mtu;
  }
  esp32xx_bt_gatts_create_sessions(ce);
  return 0;
}

int esp32xx_bt_gatts_event(const struct ble_gap_event *ev, void *arg) {
  int ret = 0;
  ...
  switch (ev->type) {
    case BLE_GAP_EVENT_CONNECT: {
      ...
      struct esp32xx_bt_gatts_connection_entry *ce = calloc(1, sizeof(*ce));
      if (ce == NULL) {
      ...
      esp32xx_bt_addr_to_mgos(&cd.peer_ota_addr, &ce->gc.addr);
      SLIST_INIT(&ce->pending_nm);
      STAILQ_INIT(&ce->pending_inds);
      SLIST_INSERT_HEAD(&s_conns, ce, next);
      ble_gattc_exchange_mtu(conn_id, esp32xx_bt_gatts_mtu_event, ce);
      break;
    }
    case BLE_GAP_EVENT_DISCONNECT: {
      ...
      struct esp32xx_bt_gatts_connection_entry *ce = find_connection(conn_id);
      if (ce == NULL) break;
      SLIST_REMOVE(&s_conns, ce, esp32xx_bt_gatts_connection_entry, next);
      while (!SLIST_EMPTY(&ce->sessions)) {
        struct esp32xx_bt_gatts_session_entry *sse = SLIST_FIRST(&ce->sessions);
        esp32xx_bt_gatts_close_session(sse);
      }
      struct esp32xx_bt_gatts_pending_ind *pi, *pit;
      STAILQ_FOREACH_SAFE(pi, &ce->pending_inds, next, pit) {
        esp32xx_bt_gatts_pending_ind_free(pi);
      }
      struct esp32xx_bt_gatts_pending_read *pr, *prt;
      SLIST_FOREACH_SAFE(pr, &ce->pending_reads, next, prt) {
        esp32xx_bt_gatts_pending_read_free(pr);
      }
      free(ce);
      break;
    }
    case BLE_GAP_EVENT_ENC_CHANGE: {
      ...
      struct esp32xx_bt_gatts_connection_entry *ce = find_connection(ch);
      if (ce == NULL) break;
      ce->sec_in_flight = false;
      LOG(LL_DEBUG, ("ENC_CHANGE %s ch %d st %d e %d a %d b %d ks %d",
                     esp32xx_bt_addr_to_str(&cd.peer_ota_addr, buf1), ch,
      ...
      while (!SLIST_EMPTY(&ce->pending_nm)) {
        struct esp32xx_bt_gatts_pending_nm *pnm = SLIST_FIRST(&ce->pending_nm);
        SLIST_REMOVE_HEAD(&ce->pending_nm, next);
        struct mgos_bt_gatts_notify_mode_arg *narg = &pnm->nmarg;
        uint16_t ah = narg->handle;
        struct esp32xx_bt_service_attr_info *ai = NULL;
        struct esp32xx_bt_gatts_session_entry *sse = find_session(ch, ah, &ai);
        LOG(LL_DEBUG,
            ("NOTIFY_MODE ch %d ah %d %s/%s %d", ch, ah,
             mgos_bt_uuid_to_str(&narg->svc_uuid, buf1),
             mgos_bt_uuid_to_str(&narg->char_uuid, buf2), narg->mode));
        esp32xx_bt_gatts_call_handler(sse, ai, MGOS_BT_GATTS_EV_NOTIFY_MODE,
                                      narg);
        memset(pnm, 0, sizeof(*pnm));
        free(pnm);
      }
    ...
    case BLE_GAP_EVENT_MTU: {
      ...
      // and in case of success it duplicates esp32xx_bt_gatts_mtu_event.
      break;
    }
    case BLE_GAP_EVENT_SUBSCRIBE: {
      ...
      if (sse->se->sec_level > 0 ||
        ...
        struct ble_gap_sec_state *ss = &cd.sec_state;
        if (!ss->encrypted) {
          struct esp32xx_bt_gatts_pending_nm *pnm = calloc(1, sizeof(*pnm));
          pnm->nmarg = narg;
    ...
    case BLE_GAP_EVENT_NOTIFY_TX: {
      ...
      if (pi->is_ind) {
        ...
        esp32xx_bt_gatts_call_handler(sse, ai, MGOS_BT_GATTS_EV_IND_CONFIRM,
                                      &ic_arg);
      } else {
      ...
      esp32xx_bt_gatts_send_next_ind_locked(ce);
      break;
    }
  }
  esp32xx_bt_runlock();
...
bool esp32xx_bt_gatts_send_resp_data(struct ble_gatt_access_ctxt *ctxt,
                                     struct esp32xx_bt_gatts_session_entry *sse,
  ...
  if (pr->read_offset >= rdl) {
    SLIST_REMOVE(&sse->ce->pending_reads, pr, esp32xx_bt_gatts_pending_read,
                 next);
    esp32xx_bt_gatts_pending_read_free(pr);
  }
  return rdl;
...
static int esp32_gatts_attr_access_cb(uint16_t ch, uint16_t ah,
  ...
  switch (ctxt->op) {
    ...
    case BLE_GATT_ACCESS_OP_READ_DSC: {
      if (esp32xx_bt_gatts_send_resp_data(ctxt, sse, ch, ah, false) > 0) {
      ...
      struct esp32xx_bt_service_attr_info *cai = arg;
      ...
      enum mgos_bt_gatt_status st =
          esp32xx_bt_gatts_call_handler(sse, ai, MGOS_BT_GATTS_EV_READ, &rarg);
      res = esp32_gatts_get_att_err(st);
      size_t rdl = 0;
      if (res == 0) {
        rdl = esp32xx_bt_gatts_send_resp_data(ctxt, sse, ch, ah, true);
      }
      if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
        LOG(LL_DEBUG,
    ...
    case BLE_GATT_ACCESS_OP_WRITE_DSC: {
      struct mg_str data = esp32xx_bt_mbuf_to_flat(ctxt->om);
      ...
      enum mgos_bt_gatt_status st =
          esp32xx_bt_gatts_call_handler(sse, ai, MGOS_BT_GATTS_EV_WRITE, &warg);
      if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
...
static int esp32xx_bt_register_service(
    struct esp32xx_bt_gatts_service_entry *se) {
  int rc = 0;
...
static bool esp32xx_bt_register_services(void) {
  char buf[MGOS_BT_UUID_STR_LEN];
  ...
  struct esp32xx_bt_gatts_service_entry *se;
  SLIST_FOREACH(se, &s_svcs, next) {
    rc = esp32xx_bt_register_service(se);
    if (rc != 0) {
...
bool mgos_bt_gatts_register_service(const char *svc_uuid,
                                    ...
                                    void *handler_arg) {
  bool res = false;
  struct esp32xx_bt_gatts_service_entry *se = calloc(1, sizeof(*se));
  if (se == NULL) goto out;
  ...
  struct esp32xx_bt_service_attr_info *ai = se->attrs, *last_chr = NULL;
...
bool mgos_bt_gatts_unregister_service(const char *uuid_str) {
  ...
  SLIST_REMOVE(&s_svcs, se, esp32xx_bt_gatts_service_entry, next);
  se->deleting = true;
...
void mgos_bt_gatts_send_resp_data(struct mgos_bt_gatts_conn *gsc,
  ...
  esp32xx_bt_rlock();
  struct esp32xx_bt_gatts_session_entry *sse = find_session_by_gsc(gsc);
  if (sse == NULL) goto out;
  struct esp32xx_bt_gatts_pending_read *pr = NULL;
  SLIST_FOREACH(pr, &sse->ce->pending_reads, next) {
...
void mgos_bt_gatts_notify(struct mgos_bt_gatts_conn *gsc,
  ...
  struct esp32xx_bt_gatts_session_entry *sse = find_session_by_gsc(gsc);
  if (sse == NULL) return;
  struct esp32xx_bt_gatts_pending_ind *pi = calloc(1, sizeof(*pi));
  if (pi == NULL) return;
  pi->handle = handle;
  ...
  esp32xx_bt_gatts_send_next_ind_locked(sse->ce);
  esp32xx_bt_runlock();
}
...
void mgos_bt_gatts_notify_uuid(struct mgos_bt_gatts_conn *gsc,
  ...
  for (uint16_t i = 0; i < se->num_attrs; i++) {
    struct esp32xx_bt_service_attr_info *ai = &se->attrs[i];
...
bool esp32xx_bt_gatts_start(void) {
  return esp32xx_bt_register_services();
}
